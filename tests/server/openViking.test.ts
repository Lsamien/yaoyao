...
describe('OpenViking memory provider', () => {
  ...
  it('uses context without falling back to local files', async () => {
    ...
    const { acknowledgeMemory, memoryDelta, memoryResetReason } = await import('../../src/server/workspaceMemoryContext')
    const state = acknowledgeMemory(context)
    const input = { requestId: randomUUID(), scope: 'agent' as const, agentId: a.id, content: '远端新增事实', tier: 'profile' as const }
    const added = await knowledge.writeMemory(owner, input)
    const updated = await knowledge.context(owner, a.id)
    expect(memoryResetReason(updated, state)).toBeUndefined()
    expect(memoryDelta(updated, state)).toContain('远端新增事实')
    expect(memoryDelta(updated, state)).not.toContain('上下文事实')
    await knowledge.forget(owner, { ...input, requestId: randomUUID(), id: added.id, expectedRevision: added.revision })
    expect(memoryResetReason(await knowledge.context(owner, a.id), acknowledgeMemory(updated, state))).toBe('memory_changed_or_removed')
