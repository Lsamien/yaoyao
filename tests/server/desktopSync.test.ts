...
function runtime(root: string, commit = 'b'.repeat(40), version = '0.3.32', ancestors = ['a'.repeat(40)]) {
  mkdirSync(join(root, 'bin'), { recursive: true }); mkdirSync(join(root, 'ui'), { recursive: true })
...
describe('transactional service switching', () => {
  ...
  it('explicitly overwrites an unrelated same-version build, preserving data and bypassing an old sync receipt', async () => {
    ...
    await expect(synchronizeDesktop({...f,runtimeRoot:f.target})).rejects.toMatchObject({code:'service_build_unknown'})
    expect(f.counts()).toEqual({stops:0,starts:0})
    const target=verifyRuntimePackage(f.target)
    writeJSON(join(f.home,'updates','desktop-sync.json'),{artifactDigest:target.artifactDigest,action:'newer'})
    const result=await synchronizeDesktop({...f,runtimeRoot:f.target,force:true})
    expect(result.action).toBe('overwrite');expect(result.current.artifactDigest).toBe(target.artifactDigest)
    expect(f.counts()).toEqual({stops:1,starts:1})
    expect(readFileSync(join(f.home,'settings.json'),'utf8')).toBe('original')
    const database=new DatabaseSync(join(f.home,'workspace.sqlite3'),{readOnly:true})
    expect(database.prepare('SELECT value FROM messages').get()?.value).toBe('before');database.close()
    expect(existsSync(JSON.parse(readFileSync(join(f.home,'updates','last-success.json'),'utf8')).snapshot)).toBe(true)
    expect((await synchronizeDesktop({...f,runtimeRoot:f.target})).action).toBe('already-synced')
    expect(f.counts()).toEqual({stops:1,starts:1})
  })
  it('does not use force to cross versions, bypass idle checks or accept a damaged package', async () => {
    const f=fixture()
    await expect(synchronizeDesktop({...f,runtimeRoot:f.target,force:true})).rejects.toMatchObject({code:'service_force_version_mismatch'})
    runtime(f.old,'c'.repeat(40),'0.3.32',[])
    f.driver.quiesce=async()=>{throw new Error('busy')}
    await expect(synchronizeDesktop({...f,runtimeRoot:f.target,force:true})).rejects.toThrow('busy')
    expect(f.counts()).toEqual({stops:0,starts:0})
    writeFileSync(join(f.target,'server.mjs'),'broken')
    await expect(synchronizeDesktop({...f,runtimeRoot:f.target,force:true})).rejects.toThrow('校验失败')
    expect(f.counts()).toEqual({stops:0,starts:0})
  })
  it('restores the previous same-version build and its data when a forced replacement fails', async () => {
    const f=fixture();runtime(f.old,'c'.repeat(40),'0.3.32',[]);f.fail()
    await expect(synchronizeDesktop({...f,runtimeRoot:f.target,force:true})).rejects.toThrow('wrong running build')
    expect(currentRelease(f.releaseRoot)).toBe(f.old)
    expect(readFileSync(join(f.home,'settings.json'),'utf8')).toBe('original')
    expect(existsSync(join(f.home,'updates','desktop-sync.json'))).toBe(false)
  })
