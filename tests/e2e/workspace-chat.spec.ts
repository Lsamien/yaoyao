...
test('an idle Web viewer follows an iOS-origin run and restores thinking after reload', async ({ page }, testInfo) => {
  ...
  await expect(typing.locator('.agent-avatar')).toBeVisible()
  await expect(typing.locator('.thinking-indicator__dot')).toHaveCount(3)
  await expect(typing).toContainText('秒')
...
test('created roles and editable teams share a durable chat list without plugin calls', async ({
  ...
  await expect(page.getByRole('menu', { name: '账号菜单' }).getByRole('menuitem')).toHaveText(['我的设置', '关于', '检测更新', '帮助', '进入聊天模式'])
  await page.getByRole('menuitem', { name: '关于', exact: true }).click()
  const about = page.getByRole('dialog', { name: '关于夭夭 AI', exact: true })
  await expect(about).toBeVisible()
  await expect(about.locator('.about-version')).toHaveText(/^版本 \d+\.\d+\.\d+$/)
  await expect(page.getByRole('dialog', { name: '我的设置', exact: true })).toHaveCount(0)
  await page.screenshot({ path: testInfo.outputPath('independent-about.png') })
  await page.keyboard.press('Escape')
  await expect(about).toHaveCount(0)
  await rail.getByRole('button', { name: '工具', exact: true }).click()
  await page.getByRole('menuitem', { name: '已连接应用', exact: true }).click()
  const apps = page.getByRole('dialog', { name: '已连接应用', exact: true })
  await expect(apps).toBeVisible()
  await expect(apps.getByRole('tab', { name: 'MCP 服务', exact: true })).toBeVisible()
  await expect(page.locator('.settings-sidebar')).toHaveCount(0)
  await page.screenshot({ path: testInfo.outputPath('independent-apps.png') })
  await apps.getByRole('button', { name: '关闭已连接应用', exact: true }).click()
  await rail.getByRole('button', { name: '工具', exact: true }).click()
  await page.getByRole('menuitem', { name: '自动化', exact: true }).click()
  await expect(page).toHaveURL(/\/conversations\/automations(?:\?.*)?$/)
  await expect(page.getByRole('main', { name: '自动化工作区' })).toBeVisible()
  ...
  const settings = page.getByRole('dialog', { name: '我的设置', exact: true })
  await settings.getByRole('button', {name:'账号资料',exact:true}).click()
  await expect(settings.locator('.account-avatar-card')).toContainText('fixture')
  await expect(settings.locator('.account-avatar-card .account-initial-avatar')).toHaveText('F')
  ...
  await expect(settings.locator('.account-avatar-card .account-initial-avatar img')).toBeVisible()
  ...
  await settings.getByRole('button', { name: '关闭我的设置', exact: true }).click()
  await rail.locator('.sidebar-account-switcher__main').click()
  await page.getByRole('menuitem', {name:'我的设置',exact:true}).click()
  await settings.getByRole('button', { name: '身份与头像', exact: true }).click()
  ...
  await expect(settingsMenu.getByRole('menuitem')).toHaveText(['我的设置', '关于', '检测更新', '帮助', '进入 Bot 模式'])
  await page.screenshot({ path: testInfo.outputPath('chat-settings-mode-menu.png') })
  await settingsMenu.getByRole('menuitem', { name: '我的设置', exact: true }).click()
  await expect(page.getByRole('dialog', { name: '我的设置', exact: true })).toBeVisible()
  await page.getByRole('button', { name: '关闭我的设置', exact: true }).click()
  ...
  await rail.locator('.sidebar-account-switcher__main').click()
  await page.getByRole('menuitem', { name: '进入聊天模式', exact: true }).click()
  await expect(page).toHaveURL(/\/chat$/)
  await page.goto('/')
  await expect(page).toHaveURL(/\/chat$/)
...
test('subaccount allocation and Bot-only navigation work through the browser', async ({ page, browser }, testInfo) => {
  ...
  await expect(child.getByRole('menu', { name: '账号菜单' }).getByRole('menuitem')).toHaveText(['我的设置', '关于', '帮助'])
...
test('self-registered subaccount approval supports Bot replies, reload and isolation', async ({ page, browser }, testInfo) => {
  const childContext = await browser.newContext({ viewport: { width: 1280, height: 800 } })
  const child = await childContext.newPage()
  const username = `registered-${Date.now()}`
  const denied: string[] = []
  child.on('response', response => { if (response.status() === 403 && response.url().includes('/api/app/')) denied.push(new URL(response.url()).pathname) })
  try {
    await child.goto('/conversations')
    await child.getByRole('button', { name: '没有账号？注册子账号' }).click()
    await child.getByRole('textbox', { name: '账号', exact: true }).fill(username)
    await child.getByLabel('密码', { exact: true }).fill('child-password')
    await child.getByLabel('确认密码', { exact: true }).fill('child-password')
    await child.getByRole('button', { name: '注册子账号', exact: true }).click()
    await expect(child.getByRole('status')).toContainText('注册成功')
    await child.getByLabel('密码', { exact: true }).fill('child-password')
    await child.getByRole('button', { name: '登录', exact: true }).click()
    await expect(child.getByRole('alert')).toContainText('尚未开通')
    await login(page)
    await page.locator('.desktop-sidebar .sidebar-account-switcher__main').click()
    await page.getByRole('menuitem', { name: '我的设置', exact: true }).click()
    const settings = page.getByRole('dialog', { name: '我的设置', exact: true })
    await settings.getByRole('button', { name: '用户与权限', exact: true }).click()
    const row = settings.locator('.user').filter({ hasText: username })
    await expect(row).toContainText('待管理员开通')
    await expect(row.getByRole('button', { name: '分配并开通' })).toBeDisabled()
    await row.getByRole('listbox').selectOption('default')
    await row.getByRole('button', { name: '分配并开通' }).click()
    ...
    const dialog = child.getByRole('dialog')
    await expect(dialog.getByLabel('基础机器人').locator('option')).toHaveCount(1)
    await dialog.getByRole('textbox', { name: '名称', exact: true }).fill('注册验收助手')
    await dialog.getByRole('button', { name: '保存', exact: true }).click()
    await expect(child.getByRole('heading', { name: '注册验收助手', exact: true })).toBeVisible()
    const composer = child.getByRole('textbox', { name: '输入消息，Enter 发送，Shift + Enter 换行' })
    await composer.fill('注册后验证 Bot 回复')
    await composer.press('Enter')
    await expect.poll(async () => {
      const id = new URL(child.url()).pathname.split('/').at(-1)
      const response = await child.request.get(`/api/app/conversations/${id}/messages`)
      const value = await response.json()
      return value.messages?.some((m: any) => m.role === 'assistant' && m.status === 'complete' && m.content)
    }).toBe(true)
    await expect(child.getByText('会话独立保存', { exact: true })).toBeVisible()
    await childContext.setOffline(true)
    await expect(child.getByText('网络已断开，恢复连接后将自动同步', { exact: true })).toBeVisible()
    await childContext.setOffline(false)
    await expect(child.getByText('网络已断开，恢复连接后将自动同步', { exact: true })).toBeHidden({ timeout: 15_000 })
    await child.reload()
    await expect(child.getByRole('heading', { name: '注册验收助手', exact: true })).toBeVisible()
    await expect(child.getByText('注册后验证 Bot 回复', { exact: true }).first()).toBeVisible()
    expect((await (await page.request.get('/api/app/agents')).json()).agents.some((a: any) => a.name === '注册验收助手')).toBe(false)
    expect(denied).toEqual([])
    await child.screenshot({ path: testInfo.outputPath('registered-subaccount-bot.png'), fullPage: true })
