...
afterEach(() => { wrapper?.unmount(); wrapper = undefined; vi.restoreAllMocks(); vi.mocked(apiRequest).mockReset(); HTMLDialogElement.prototype.showModal = originalShowModal; HTMLDialogElement.prototype.close = originalClose })
...
it.each(['direct', 'group'] as const)('opens and saves the menu target %s while another chat is selected', async kind => {
  ...
  vi.mocked(apiRequest).mockImplementation(async path => {
    ...
    const conversation = conversations.find(c => path === `/api/app/conversations/${c.id}`)
  ...
  wrapper = shallowMount(ConversationsView, { attachTo: document.body, global: {
    ...
    stubs: { WorkspaceShell: { template: '<div><slot name="sidebar" /><slot /></div>' }, Teleport: false },
  } })
  await flushPromises()
