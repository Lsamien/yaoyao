...
- Bot 的组队和协作由服务端统一开放，资料中只展示权限状态；旧客户端提交的单 Bot 组队、协作开关不会覆盖服务端策略。“自动记录长期事实”仍可单独暂停。
- 持久 Bot 可通过 `workspace_get_self_rules` 读取自己的名称、完整提示词、角色规则和版本，再用 `workspace_update_self_rules` 修改名称、描述、长期提示词、职责、边界、语气和行动偏好。更新校验 `expectedRevision`，新规则从后续轮次生效；不修改基础 Hermes Profile、模型或工具权限。
- Bot 创建成员时可填写对方的初始提示词和角色规则。创建完成后，创建者不能再修改对方规则；原有改名、归档权限保持不变。成员仍可修改自己的规则，用户也可继续在客户端编辑。临时助手不开放自我修改，记忆维护仍遵守下文的独立权限。
...
记忆分为 `profile`（基础事实）、`log`（历史）和 `note`（仅搜索）三层。Bot、项目、用户三个范围共用 16,000 字符的提示预算，包含说明、范围标题和来源标识。先轮流选取各范围的基础事实，再按各范围的最近更新时间选取历史；只放入完整条目。超出预算的事实仍保留，可通过 `workspace_memory_search` 查询。

后台提炼请求包含提示、已有记忆和本轮证据，总长度最多 60,000 字符，低于工具桥的 65,000 字符限制。证据按序列化后的 JSON 长度计算，保留用户消息优先级和完整 JSON 结构，不通过截断 JSON 字符串控制长度。

本地文件搜索按正文关键词匹配。启用 OpenViking 后保留授权范围内的语义命中和关键词命中，不再要求语义结果包含查询原文。
