...
- 安装不会自动重启 Hermes。请在空闲时重启 Dashboard 服务（默认 9119），再点击“重新检查”。正在执行任务时暂不可安装。
...
管理员 API：`GET /api/app/admin/hermes-bridge` 与 `POST /api/app/admin/hermes-bridge/install`，安装参数仅为 `profile` 和可选的 `enable`；不接受客户端传入目录、命令或 Python 路径。
