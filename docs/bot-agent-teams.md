...
Agent 可以查看可用基础 Agent、复用已有成员、创建持久成员和团队、启动交付目标、读取进展和结果。新团队自动加入发起者并将其设为管理员。普通聊天保留 @协作；目标执行使用结构化分派和复核，避免同一工作再通过 @重复派发。成员、团队和任务写入当前账号的 `workspace.sqlite3`，发布既有变更事件供客户端同步。
