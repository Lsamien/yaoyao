...
| 标签 | 架构 |
| --- | --- |
| `v0.4.18-amd` | `linux/amd64` |
| `v0.4.18` | `linux/amd64`、`linux/arm64`，拉取时自动匹配 |
| `latest` | 最新稳定通用镜像，当前指向与 `v0.4.18` 相同的镜像 |

```sh
docker pull samienluo/yaoyao:v0.4.18
# 需要固定 AMD64 时：
docker pull samienluo/yaoyao:v0.4.18-amd
```
...
| 配置 | 用途 |
| --- | --- |
| `HERMES_YAOYAO_IMAGE` | 远程镜像可设为 `samienluo/yaoyao:v0.4.18` 或 `samienluo/yaoyao:latest`；未设置时使用本地构建名称 |
...
```dotenv
HERMES_YAOYAO_IMAGE=samienluo/yaoyao:v0.4.18
