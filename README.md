...
当前 Web 与 macOS 桌面发布版本：**v0.4.72**。桌面版提供 Apple Silicon（arm64）正式 DMG，并附 Windows x64 未签名测试安装包，详见 [发布说明](docs/releases/v0.4.72.md)。
...
远程电脑：其他 Mac 上的夭夭 App 启动时可选择「登录远程」或「进入本地」；登录远程用服务器地址加账号密码登录（与手机端一致的凭据方式），窗口直接使用远程服务器（15300）的夭夭界面，本机不运行 Web 服务；管理员账号登录同时把这台 Mac 注册为可远程控制的电脑，供服务器上的 Bot 操作。每个机器人可并列启用本地虚拟机、云端和多台 Mac 的电脑控制，工具按在线情况同时可用。详见 [服务器与电脑](docs/desktop-hosts.md)。
...
在 [v0.4.72 发布页](https://github.com/Lsamien/yaoyao/releases/tag/v0.4.72) 下载 `Yaoyao-0.4.72-arm64.dmg`，将“夭夭”拖到应用程序后打开。首次启动会核对并同步配套本机 Web 服务；普通退出保留后台运行，“停止后台服务并退出”可完整停止本应用管理的后台。详细说明见 [macOS App](docs/macos-app.md)。
...
| 镜像 | 架构 |
| --- | --- |
| `samienluo/yaoyao:v0.4.72-amd` | AMD64 |
| `samienluo/yaoyao:v0.4.72` | 自动匹配 AMD64 / ARM64 |
| `samienluo/yaoyao:latest` | 最新稳定版通用镜像，当前与 `v0.4.72` 相同 |

准备好 Docker Engine/Compose 和容器可访问的 Hermes 9119 服务，在仓库目录将 `docker.env.example` 复制为 `docker.env`。配置上游地址，并设置 `HERMES_YAOYAO_IMAGE=samienluo/yaoyao:v0.4.72`（或 `samienluo/yaoyao:latest`），即可使用远程镜像：
...
需要随 Docker Web 部署电脑环境，可使用 `compose.desktops.yaml`（两台标准桌面）或 `compose.desktops.cursor.yaml`（一台 Cursor Universal 开发桌面）。Cursor 方案基于 `public.ecr.aws/k0i0n2g5/cursorenvironments/universal:sand-box-latest` 加入夭夭桌面桥接层，配置与启动命令见 [Cursor 桌面部署](docs/docker-install.md#cursor-universal-可选配置)。
