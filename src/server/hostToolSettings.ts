...
export function parseHostTools(value: unknown): HostToolSettings {
  ...
  if (input.managedBrowser !== undefined && typeof input.managedBrowser !== 'boolean')
    throw new HttpError(400, '浏览器开关无效', 'invalid_host_tools')
  ...
  return {
    ...
    managedBrowser: input.managedBrowser === true,
...
export function readHostTools(home: string): HostToolSettings {
  ...
  if (!existsSync(path)) return { denyServerTools: false, approvalPolicy: 'ask', ...COMPUTERS_ON, managedBrowser: false, fileTransferMaxMiB: FILE_TRANSFER_DEFAULT_MIB }
