...
export function remoteAgentExports(store: WorkspaceStore, nodes: WorkspaceNodes, pairings: NodePairingStore, auth: LocalAuthStore): Koa.Middleware {
  return async (ctx,next) => {
    ...
    if (!agentId) {
      if (ctx.method!=='GET') throw new HttpError(405,'只允许读取远端 Agent','method_not_allowed')
    ...
    if (agent.archived) throw new HttpError(410,'远端 Agent 已归档','remote_agent_archived')
    ...
    if (!Number.isInteger(hops) || hops<0 || hops>=8) throw new HttpError(508,'远端 Agent 引用链过长或循环','agent_reference_loop')
    ...
    const requireBinding=(id:string)=>{
      ...
      if (!binding || binding.profile!==agent.profile) throw new HttpError(403,'会话不属于这个 Agent 引用','agent_session_forbidden')
    }
    const transform=(frame:Record<string,any>)=>{
      ...
      if ((frame.method==='prompt.submit'||frame.method==='session.steer') && !agent.remoteAgentId) {
        ...
        params.text=`你是 ${agent.name}。远端 Agent 的角色与规则（版本 ${agent.revision}）：\n${agent.instructions}\n\n${params.text}`
      }
    ...
    if (path.startsWith('/api/realtime/')) {
      ...
      if (selectedChannel && !store.get(owner,'agent-export-channel',bindingKey(selectedChannel)))
        throw new HttpError(403,'通道不属于这个 Agent 引用','agent_channel_forbidden')
      const receiptID=/^\/api\/realtime\/commands\/([^/]+)$/.exec(path)?.[1]
      if (receiptID && !store.get(owner,'agent-export-command',bindingKey(receiptID)))
        throw new HttpError(403,'命令不属于这个 Agent 引用','agent_command_forbidden')
      ...
      if (!allowed) throw new HttpError(404,'Agent 实时路径不存在','not_found')
      let body: any
      if (ctx.method==='POST') {
        ...
        else if(body.channel!=='chat') throw new HttpError(403,'Agent 仅支持聊天通道','invalid_channel')
      }
    ...
    if (ctx.method!=='GET') throw new HttpError(405,'Agent 引用只允许读取文件与会话','method_not_allowed')
    ...
    else if(path!=='/api/files/download') throw new HttpError(403,'路径不属于 Agent 引用','agent_path_forbidden')
