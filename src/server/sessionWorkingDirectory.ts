...
export async function configuredWorkingDirectory(request: Request, profile: string): Promise<string | undefined> {
  ...
  if (response.status !== 200) {
    throw new HttpError(502, '无法读取机器人的 terminal.cwd，未开始执行', 'working_directory_unavailable')
  }
  let config: any
  try { config = JSON.parse(response.body.toString()) } catch {
    throw new HttpError(502, '机器人工作目录配置响应无效', 'invalid_working_directory')
  }
  ...
  if (typeof raw !== 'string' || !raw.trim() || raw.length > 4096 || /[\u0000-\u001f\u007f]/.test(raw)) {
    throw new HttpError(502, '机器人的 terminal.cwd 配置无效', 'invalid_working_directory')
  }
  return raw.trim()
}
