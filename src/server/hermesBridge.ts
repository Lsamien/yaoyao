...
import type {HermesBridgeStatus,HermesBridgeProfileStatus,HermesBridgeInstallResult} from '../shared/hermesBridge.js'
...
interface Options {home?:string;python?:string;assetsRoot?:string;isIdle?:()=>boolean;run?:(python:string,args:string[])=>Promise<string>}
...
export class HermesBridgeManager {
  ...
  constructor(readonly config:ServerConfig,readonly session:Pick<UpstreamServiceSession,'request'>,readonly options:Options={}){
  ...
  get idleForUpdate(){return !this.installing}
  ...
  async status():Promise<HermesBridgeStatus>{
    ...
    for(let i=0;i<ordered.length;i+=4)profiles.push(...await Promise.all(ordered.slice(i,i+4).map(async profile=>{
      ...
      const result:HermesBridgeProfileStatus={profile,state:'unavailable',message:'无法连接 Hermes 或读取工具桥状态',canInstall:this.available&&local?.valid===true&&!this.installing&&(this.options.isIdle?.()??true),
    ...
    return {endpoint:this.config.upstream.origin,local:this.local,...(this.mapped?{mapped:true}:{}),bundledVersion:this.bundledVersion,checkedAt:Date.now(),profiles,...(this.installing?{installing:this.installing}:{}),
  ...
  async install(value:unknown):Promise<HermesBridgeInstallResult>{
    ...
    try{
      ...
      const output=await this.run(this.python,[this.script,'--hermes-home',this.home,'--profile',parsed.data.profile,...(parsed.data.enable?['--enable']:[])])
      result=JSON.parse(output.trim())
      if(result.profile!==parsed.data.profile||typeof result.backup!=='string')throw new Error('invalid installer result')
    }catch(error){
      if(error instanceof HttpError)throw error
      throw new HttpError(409,'工具桥安装失败，请检查目录权限后重新检查状态。','hermes_bridge_install_failed')
    }finally{this.installing=undefined}
    return {profile:parsed.data.profile,backup:result.backup,message:result.disabled?'插件文件已更新，保留了禁用设置。':'工具桥已安装并启用。请在空闲时重启 Hermes Dashboard 服务，然后重新检查。',status:await this.status()}
  }
}
