...
function sessionLocation(id: string, profile: string) {
  return { path: `/history/${encodeURIComponent(id)}`, query: { profile } }
}
...
async function chooseSession(id: string) {
  ...
  await ensureRouteProfile(profile)
  if (chat.activeProfileName !== profile) await chat.loadSessions(profile, 'history')
...
watch(() => auth.activeProfile?.name, async (profile, previous) => {
  ...
  if (requestedId && routeProfile() === profile) {
    await Promise.allSettled([chat.loadSessions(profile, 'history'), chat.loadUnread(profile)])
    await chat.selectSession(requestedId, profile)
    return
  }
  await router.replace({ path: '/history', query: { profile } })
  await Promise.allSettled([chat.loadSessions(profile, 'history'), chat.loadUnread(profile)])
})
