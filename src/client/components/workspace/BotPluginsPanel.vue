...
const props = defineProps<{ connectedOnly?: boolean }>()
const emit = defineEmits<{ 'dirty-change': [value: boolean] }>()
const base = '/api/app/bot-tools'
const surface = ref<'apps' | 'mcp'>('apps'), view = ref(props.connectedOnly ? 'connected' : 'marketplace')
...
<template>
  <section class="bot-plugins" aria-label="Bot 插件管理">
    ...
    <div v-if="!connectedOnly" class="plugin-tabs" role="tablist" aria-label="插件类型">
    ...
    <template v-else-if="surface === 'mcp' && !connectedOnly">
