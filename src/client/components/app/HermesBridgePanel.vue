...
async function load(){
  if(installing.value||restarting.value)return
  clearTimeout(timer)
  ...
  try{
    ...
    if(result.installing||result.dashboard?.restarting)timer=setTimeout(()=>void load(),1500)
...
async function install(profile:HermesBridgeProfileStatus){
  if(installing.value||checking.value||restarting.value||status.value?.dashboard?.restarting||!profile.canInstall)return
  clearTimeout(timer);++generation;installing.value=profile.profile;error.value='';notice.value='';backup.value=''
  try{
...
async function restart(){
  if(checking.value||installing.value||restarting.value||!status.value?.dashboard?.canRestart)return
  clearTimeout(timer);++generation;restarting.value=true;error.value='';notice.value=''
  try{
    const result=await restartHermesDashboard()
    if(disposed)return
    status.value=result.status;notice.value=result.message
  }catch(cause){
    if(disposed)return
    error.value=cause instanceof Error?cause.message:'Hermes Dashboard 重启失败'
    // Refresh capabilities after failure without hiding the failure message.
    try{const result=await getHermesBridgeStatus();if(!disposed){status.value=result;if(result.dashboard?.restarting)timer=setTimeout(()=>void load(),1500)}}catch{}
  }finally{if(!disposed)restarting.value=false}
}
onMounted(()=>void load())
...
<template>
  <section class="hermes-bridge" aria-label="工具桥插件" :aria-busy="checking || !!installing || !!status?.installing || restarting || !!status?.dashboard?.restarting">
    <header>
      <div><h4>工具桥插件</h4><p>连接 Bot 的电脑、技能和应用工具。默认及其他 Profile 需分别安装。</p></div>
      <button type="button" :disabled="checking || !!installing || restarting" @click="load">{{ checking ? '检查中…' : '重新检查' }}</button>
    </header>
    ...
    <template v-if="status">
      ...
      <div class="profiles">
        <article v-for="profile in status.profiles" :key="profile.profile" class="profile">
          ...
          <button v-if="status.local" type="button" class="install" :disabled="checking || !!installing || restarting || !!status.dashboard?.restarting || !profile.canInstall" :aria-label="`${actionLabel(profile)}：${profileLabel(profile.profile)}`" @click="install(profile)">
