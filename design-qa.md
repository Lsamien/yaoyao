...
- 视觉依据：`docs/design/desktop-onboarding/approved-design.png`，1672 × 941 px，包含同一设计的本机安装、远程登录两个状态。它是展示画板，不是两个运行窗口。
- 浏览器实现：`preview-local-install.png`、`preview-local-account.png`、`preview-remote.png`，均在 `docs/design/desktop-onboarding/`。使用生产 `boot.html`、CSS、JS 和状态控制器，服务行为由预览适配器模拟。
- Electron 实现：同目录 `remote-login.png`、`remote-dark.png`、`remote-narrow.png`；真实主窗口、IPC、模拟 HTTP 服务器完成认证。
- 默认核对视口：1280 × 832 CSS px；Electron 原生标题栏下内容区约 1280 × 828 px。截图密度为 1；使用完整页面截图时高度可能比视口略长。
- 窄窗口：720 × 560（Electron），390 × 844（浏览器补充响应式检查），均无横向溢出，内容可纵向滚动。
- 归一化：分别比较设计画板中两个白色应用区域与实现的页面内容区域，排除画板标题、外围留白和说明。保留原始截图，未将不同屏幕宽度的比较称为逐像素复刻。宽窗口采用 820 px 内容列，延续参考的单列步骤、并排服务器选择和全宽主按钮。
- 全视图对照：在同一次视觉检查输入中打开设计画板、安装中截图与远程登录截图；另检查创建管理员、深色及窄窗口截图。
- 重点区域：对照服务器选项、连接反馈、表单标签与主按钮。全尺寸截图中这些文字清晰可读，不需要再放大裁剪。

## 核对结果

- 字体与层级：沿用系统中文字体及现有品牌图；26 px 页面标题、17 px 区块标题、13–14 px 表单文字。修复“确认密码”标签换行，中文产品名使用准确的“夭夭”。
- 间距与布局：服务器选择始终可见；准备与账号步骤由分隔线分组，当前步骤突出。默认桌面窗口下远程登录和本机创建账号的主按钮及页底控件完整可见。
- 颜色：沿用白色画布、黑色主按钮、灰色次级面和绿色成功提示；深色模式对应既有色值，错误不会仅靠颜色表达。
- 资产：复用项目品牌图片；图标来自已安装的 `@vicons/tabler`，未绘制近似图标或引入额外字体、图片服务。
- 文案与内容：本机准备绑定真实服务状态；没有可测量百分比时使用不定进度。首次初始化与已有账号由服务响应决定。准备完成后收起详情，保留成功摘要与展开入口。
- 交互：本机/远程选择、安装详情、帮助、密码显示、必填与确认密码、重新检测、地址修改后的凭据清空均有实现。登录完成后只导航原主窗口。
- 可访问性：明确标签、原生单选框、键盘焦点、44 px 输入/按钮、页内 alert/status 与减少动画设置。

## 迭代记录

1. P2：初版默认窗口的远程登录页底控件略超出视口，主按钮与服务器选择左边缘不齐。
   修复：调整步骤与区块间距，主按钮移出缩进的字段区域。重新截图 `preview-remote.png`，Electron 测试断言 footer 位于窗口内。
2. P2：本机创建管理员的“确认密码”标签换行，已完成安装列表占用过多高度。
   修复：增加标签列宽、禁止标签断行，准备完成后收起进度列表并保留详情摘要。重新截图 `preview-local-account.png`；浏览器检查 footer 底部为 809.89 px，视口高 832 px。
3. 最终检查：远程登录、本机安装中、本机账号、深色、窄窗口均无待修复的 P0/P1/P2 问题。浏览器控制台无 warning/error。

## 有意保留的差异与验证边界

- 设计图的示例百分比替换为真实阶段进度；“安装组件”改为“准备服务组件”，覆盖安装和已有环境复用。
- 当前服务器尚未检测完成时，不预先承诺创建管理员；检测后才切换为“创建管理员”或“登录账号”。
- 安装与登录使用现有真实服务逻辑；浏览器预览仅演示交互。未连接用户的真实远程服务器执行登录。
- 本次生成了桌面运行资源并验证；没有发布版本或替换用户已安装的 App。

## 实现检查清单

- [x] 单主窗口内完成服务器选择、安装准备及登录。
- [x] 登录错误、安装重试、电脑授权异常保持在页内。
- [x] 凭据绑定已检测服务器，IPC 限制在本地可信主页面。
- [x] 默认窗口、窄窗口和深色模式核对。
- [x] 服务管理、认证、生命周期集成验证与类型检查。
